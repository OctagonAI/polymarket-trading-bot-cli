...
Credentials are saved to `.polymarket-trading-bot-cli/credentials/whatsapp/default/`.
...
The gateway configuration is stored at `.polymarket-trading-bot-cli/gateway.json`. It's auto-created when you run `gateway:login`.
...
Add group policy to your account in `.polymarket-trading-bot-cli/gateway.json`:
...
2. Delete the credentials:
   ```bash
   rm -rf .polymarket-trading-bot-cli/credentials/whatsapp/default
   ```
3. Run login again:
   ```bash
   bun run gateway:login
...
- Verify your phone number is in `allowFrom` in `.polymarket-trading-bot-cli/gateway.json`
- Make sure you're messaging yourself (self-chat mode)

**Debug logs:**
- Check `.polymarket-trading-bot-cli/gateway-debug.log` for detailed logs
...
3. **Clear all local data:**
   ```bash
   rm -rf .polymarket-trading-bot-cli/credentials/whatsapp/default
   rm -rf .polymarket-trading-bot-cli/gateway.json
   rm -rf .polymarket-trading-bot-cli/gateway-debug.log
   ```
